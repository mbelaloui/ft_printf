...
/*   Updated: 2018/02/11 16:02:47 by mbelalou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../test.h"

void		ft_test_wchar(const wchar_t *c)
{
	int nbr_test_ok = 0;

	int		succeed = 0;
	int		accuracy = 20;
	int		max_lenght = 1 + accuracy;
	int		min_length , precision, num_test = 0, len_char = 1;
...
/*
  ...
  ft_putstr(GREEN);
	...
	while (precision >= len_char)
	{
		...
		ft_putstr(GREEN);
	ft_putstr("-- char  with \"min_len\" and \"precision\" i and \"+\"--\n");
		ft_putstr(WHITE);
